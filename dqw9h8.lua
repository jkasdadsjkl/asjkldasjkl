...
local WhitelistedHWIDs = {
	...
	"0626fdc8-8013-492b-8cc4-0bf72a46f745"
}

local isWhitelisted = false

for _, id in pairs(WhitelistedHWIDs) do
    if HWIDplayer == id then
        isWhitelisted = true
        break
    end
end
...
Notifier.new({
	...
	Duration = 5,
	Icon = "rbxassetid://138259269816887"
})
...
local DualTab4 = Window:DrawContainerTab({ Name = "Exploits", Icon = "triangle", EnableScrolling = true })
local DualTab5 = Window:DrawContainerTab({ Name = "Utility", Icon = "box", EnableScrolling = true })
do
	local MovementTab = Window:DrawTab({ Name = "Movement", Icon = "contact", Type = "Double", EnableScrolling = true })
	local MovementLeftSection = MovementTab:DrawSection({ Name = "Movement Controls", Position = 'left', EnableScrolling = true })
	local MovementRightSection = MovementTab:DrawSection({ Name = "Advanced Movement", Position = 'right', EnableScrolling = true })
	...
	local function applyMovementSetting(settingName, value)
		local success, err = pcall(function()
			...
			showNotification('Movement', ('%s: %s'):format(settingName, tostring(value)))
		end)
		if not success then
			showNotification('Movement', 'Failed to set ' .. settingName .. ': ' .. tostring(err))
		end
	end
	MovementLeftSection:AddSlider({ Name = "Dash Speed", Min = 1, Max = 300, Default = Cfg.DashSpeed, Round = 0, Flag = "DashSpeed", Callback = function(value) Cfg.DashSpeed = value; applyMovementSetting("dashSpeed", value) end })
	MovementLeftSection:AddToggle({ Name = "No Dash Cooldown", Flag = "NoDashCooldown", Default = Cfg.NoDashCooldown, Callback = function(value) Cfg.NoDashCooldown = value; applyMovementSetting("noDashCooldown", value) end })
	MovementLeftSection:AddToggle({ Name = "No Stun", Flag = "NoStunMovement", Default = Cfg.NoStunMovement, Callback = function(value) Cfg.NoStunMovement = value; applyMovementSetting("noStun", value) end })
	MovementLeftSection:AddToggle({ Name = "No Jump Fatigue", Flag = "NoJumpFatigue", Default = Cfg.NoJumpFatigue, Callback = function(value) Cfg.NoJumpFatigue = value; applyMovementSetting("noJumpFatigue", value) end })
	MovementLeftSection:AddToggle({ Name = "No Stun On Miss", Flag = "NoStunOnMiss", Default = Cfg.NoStunOnMiss, Callback = function(value) Cfg.NoStunOnMiss = value; applyMovementSetting("noStunOnMiss", value) end })
	MovementLeftSection:AddToggle({ Name = "No Slowdown", Flag = "NoSlowdown", Default = Cfg.NoSlowdown, Callback = function(value) Cfg.NoSlowdown = value; applyMovementSetting("noSlowdown", value) end })
	...
	local function togglePlatform(state)
		...
		if state then
			createPlatform()
			disableCollisions()
			showNotification('Movement', 'Fly And Noclip On')
		else
			if platform then platform:Destroy(); platform = nil end
			restoreCollisions()
			showNotification('Movement', 'Fly And Noclip Off')
		end
	end
	local function toggleSpeed(state)
		...
		if state then
			...
			showNotification('Movement', 'Speed Hack On')
		else
			...
			showNotification('Movement', 'Speed Hack Off')
		end
	end
	updateCharacterRefs()
	cacheParts()
	...
	MovementRightSection:AddToggle({ Name = "Speed Hack", Flag = "SpeedHack", Default = Cfg.SpeedHack, Callback = toggleSpeed })
	MovementRightSection:AddSlider({ Name = "Speed Multiplier", Flag = "SpeedMultiplier", Min = 1, Max = 4, Default = Cfg.SpeedMultiplier, Round = 0, Callback = function(value) Cfg.SpeedMultiplier = value; showNotification('Movement', 'Speed: ' .. value) end })
	MovementRightSection:AddToggle({ Name = "Fly & Noclip", Flag = "FlyNoclip", Default = Cfg.FlyNoclip, Callback = togglePlatform })
end
do
	local WorldVisualsTab = DualTab2:DrawTab({ Name = "World Visuals", Type = "Double", EnableScrolling = true })
	local WorldAmbientSection = WorldVisualsTab:DrawSection({ Name = "World Ambient", Position = 'left', EnableScrolling = true })
	local CharacterColorsSection = WorldVisualsTab:DrawSection({ Name = "Character Colors", Position = 'right', EnableScrolling = true })
	local AuraSection = WorldVisualsTab:DrawSection({ Name = "Transform Aura", Position = 'left', EnableScrolling = true })
	local TrailsSection = WorldVisualsTab:DrawSection({ Name = "Trails", Position = 'right', EnableScrolling = true })
	do
		...
		WorldAmbientSection:AddColorPicker({ Name = "Ambient Color", Flag = "AmbientColor", Default = Cfg.AmbientColor, Callback = function(value) Cfg.AmbientColor = value; ApplyAmbient() end })
		WorldAmbientSection:AddColorPicker({ Name = "OutdoorAmbient Color", Flag = "OutdoorAmbientColor", Default = Cfg.OutdoorAmbientColor, Callback = function(value) Cfg.OutdoorAmbientColor = value; ApplyAmbient() end })
		WorldAmbientSection:AddToggle({ Name = "Ambient Toggle", Flag = "AmbientToggle", Default = Cfg.AmbientToggle, Callback = function(value) Cfg.AmbientToggle = value; ApplyAmbient() end })
		WorldAmbientSection:AddToggle({ Name = "OutdoorAmbient Toggle", Flag = "OutdoorAmbientToggle", Default = Cfg.OutdoorAmbientToggle, Callback = function(value) Cfg.OutdoorAmbientToggle = value; ApplyAmbient() end })
		ApplyAmbient()
	end
	do
		...
		local function storeOriginalCharacterColors(obj)
			if not originalCharacterColors[obj] then originalCharacterColors[obj] = {} end
			for _, child in ipairs(obj:GetDescendants()) do
				if child:IsA("BasePart") then originalCharacterColors[obj][child] = {Type = "BasePart", Color = child.Color}
				elseif child:IsA("ParticleEmitter") then originalCharacterColors[obj][child] = {Type = "ParticleEmitter", Color = child.Color}
				elseif child:IsA("Trail") then originalCharacterColors[obj][child] = {Type = "Trail", Color = child.Color}
				elseif child:IsA("Beam") then originalCharacterColors[obj][child] = {Type = "Beam", Color = child.Color}
				elseif child:IsA("Highlight") then originalCharacterColors[obj][child] = {Type = "Highlight", FillColor = child.FillColor, OutlineColor = child.OutlineColor}
				elseif child:IsA("PointLight") then originalCharacterColors[obj][child] = {Type = "PointLight", Color = child.Color}
				end
			end
		end
		local function changeCharacterColors(obj, newColor)
			for _, child in ipairs(obj:GetDescendants()) do
				if child:IsA("BasePart") then child.Color = newColor
				elseif child:IsA("ParticleEmitter") then child.Color = ColorSequence.new(newColor)
				elseif child:IsA("Trail") then child.Color = ColorSequence.new(newColor)
				elseif child:IsA("Beam") then child.Color = ColorSequence.new(newColor)
				elseif child:IsA("Highlight") then child.FillColor = newColor; child.OutlineColor = newColor
				elseif child:IsA("PointLight") then child.Color = newColor
				end
			end
		end
				...
				for child, colorData in pairs(originalCharacterColors[obj]) do
					if child.Parent then
						if colorData.Type == "Highlight" then
							child.FillColor = colorData.FillColor; child.OutlineColor = colorData.OutlineColor
						else
		...
		local function applyCharacterColors()
			local success, playerChar = pcall(function() return LocalPlayer:WaitForChild("Data"):WaitForChild("Character").Value end)
			if success and findCharacterForColor(playerChar) then
				for _, character in ipairs(foundCharacters) do
					if not originalCharacterColors[character] then storeOriginalCharacterColors(character) end
					changeCharacterColors(character, Cfg.CharacterColor)
				end
			end
		end
		local function resetCharacterColors()
			for _, character in ipairs(foundCharacters) do restoreCharacterColors(character) end
		end
		CharacterColorsSection:AddColorPicker({ Name = "Character Color", Flag = "CharacterColor", Default = Cfg.CharacterColor, Callback = function(color) Cfg.CharacterColor = color end })
		CharacterColorsSection:AddButton({ Name = "Apply Colors", Flag = "ApplyColorsButton", Callback = applyCharacterColors })
		CharacterColorsSection:AddButton({ Name = "Reset Colors", Flag = "ResetColorsButton", Callback = resetCharacterColors })
		local SavedData = {}
		local function ResetSavedData()
			SavedData = { Clothing = {}, AccessoryTextures = {}, MeshTextures = {}, Face = nil, HeadAccessories = {}, BodyParts = {} }
		end
		ResetSavedData()
		...
		local function ApplyMaterialEffect(char, material, color)
			...
			for _, obj in ipairs(char:GetChildren()) do
				if obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("ShirtGraphic") then
					if not SavedData.Clothing[obj] then SavedData.Clothing[obj] = obj:Clone() end; obj:Destroy()
				end
			end
			local head = char:FindFirstChild("Head")
			if head then
				local face = head:FindFirstChild("face")
				if face and not SavedData.Face then SavedData.Face = face:Clone(); face:Destroy() end
			end
			for _, acc in ipairs(char:GetChildren()) do
				if IsHeadAccessory(acc) then
					if not SavedData.HeadAccessories[acc] then SavedData.HeadAccessories[acc] = acc:Clone() end; acc:Destroy()
				end
			end
			for _, part in ipairs(char:GetDescendants()) do
				if part:IsA("BasePart") then
					if not SavedData.BodyParts[part] then SavedData.BodyParts[part] = {Material = part.Material, Color = part.Color} end
					part.Material = material; part.Color = color
				end
			end
			for _, acc in ipairs(char:GetChildren()) do
				if acc:IsA("Accessory") and acc:FindFirstChild("Handle") then
					...
					for _, d in ipairs(handle:GetDescendants()) do
						if d:IsA("Decal") or d:IsA("Texture") then
							if not SavedData.AccessoryTextures[d] then SavedData.AccessoryTextures[d] = d.Texture end; d.Texture = ""
						elseif d:IsA("SpecialMesh") then
							if not SavedData.MeshTextures[d] then SavedData.MeshTextures[d] = d.TextureId end; d.TextureId = ""
						end
					end
					handle.Material = material; handle.Color = color
				end
			end
		end
		local function RevertMaterialEffect(char)
			...
			for _, clone in pairs(SavedData.Clothing) do
				if clone and not char:FindFirstChild(clone.Name) then clone:Clone().Parent = char end
			end
			if SavedData.Face and char:FindFirstChild("Head") and not char.Head:FindFirstChild("face") then SavedData.Face:Clone().Parent = char.Head end
			for _, clone in pairs(SavedData.HeadAccessories) do
				if clone and not char:FindFirstChild(clone.Name) then clone:Clone().Parent = char end
			end
			for inst, tex in pairs(SavedData.AccessoryTextures) do if inst and inst.Parent then inst.Texture = tex end end
			for mesh, texId in pairs(SavedData.MeshTextures) do if mesh and mesh.Parent then mesh.TextureId = texId end end
			for part, data in pairs(SavedData.BodyParts) do if part and part.Parent then part.Material = data.Material; part.Color = data.Color end end
		end
		local function ReapplyMaterialEffect(char)
			if Cfg.CharacterMaterial == "None" then RevertMaterialEffect(char)
			else ApplyMaterialEffect(char, Enum.Material[Cfg.CharacterMaterial], Cfg.MaterialColor) end
		end
		...
		CharacterColorsSection:AddColorPicker({ Name = "Material Color", Flag = "MaterialColor", Default = Cfg.MaterialColor, Callback = function(value) Cfg.MaterialColor = value; if Cfg.CharacterMaterial ~= "None" then ReapplyMaterialEffect(LocalPlayer.Character) end end })
		CharacterColorsSection:AddDropdown({ Name = "Character Material", Default = Cfg.CharacterMaterial, Multi = false, Flag = "CharacterMaterial", Values = {"None", "Neon", "ForceField", "Glass", "Metal", "Wood", "Concrete", "Marble"}, Callback = function(value) Cfg.CharacterMaterial = value; ReapplyMaterialEffect(LocalPlayer.Character) end })
	end
	do
		local auraPaths = {}
		local activeAuras = {}
		...
		local function getAuraPaths()
			local paths = {}
			local success, combat = pcall(function() return LocalPlayer.PlayerScripts.Combat.Transform end)
		...
		local function ClearAuras()
			for _, aura in pairs(activeAuras) do if aura and aura.Parent then aura:Destroy() end end
			activeAuras, originalAuraColors = {}, {}
		end
		local function ProcessAuraObject(obj, mode, color)
			local function SaveOriginalAuraColor(o)
				if o:IsA("ParticleEmitter") or o:IsA("Beam") or o:IsA("Trail") or o:IsA("PointLight") or o:IsA("SpotLight") or o:IsA("SurfaceLight") or o:IsA("MeshPart") or o:IsA("Part") then
			...
			local function ApplyAuraColor(o, c)
				if o:IsA("ParticleEmitter") or o:IsA("Beam") or o:IsA("Trail") then o.Color = ColorSequence.new(c)
				elseif o:IsA("PointLight") or o:IsA("SpotLight") or o:IsA("SurfaceLight") or o:IsA("MeshPart") or o:IsA("Part") then o.Color = c
				elseif o:IsA("Highlight") then o.FillColor, o.OutlineColor = c, c end
			end
			local function ResetAuraColor(o)
				local saved = originalAuraColors[o]
				if not saved then return end
				if o:IsA("Highlight") then o.FillColor, o.OutlineColor = saved.FillColor, saved.OutlineColor
				else o.Color = saved end
			end
			if mode == "save" then SaveOriginalAuraColor(obj)
			elseif mode == "apply" and color then ApplyAuraColor(obj, color)
			elseif mode == "reset" then ResetAuraColor(obj) end
			for _, child in pairs(obj:GetChildren()) do ProcessAuraObject(child, mode, color) end
		end
			...
			for _, effectPart in pairs(folder:GetChildren()) do
				...
				if bodyPart then
					for _, effect in pairs(effectPart:GetChildren()) do
						local success, clone = pcall(function() return effect:Clone() end)
						if success and clone then
							clone.Parent = bodyPart
							table.insert(activeAuras, clone)
							ProcessAuraObject(clone, "save")
							if Cfg.AuraColor and not Cfg.RainbowAura then ProcessAuraObject(clone, "apply", Cfg.AuraColor) end
						end
					end
				end
			end
		end
		local function RecolorAuras(color) for _, aura in pairs(activeAuras) do ProcessAuraObject(aura, "apply", color) end end
		local function ResetAurasToDefault() Cfg.AuraColor = nil; for _, aura in pairs(activeAuras) do ProcessAuraObject(aura, "reset") end end
		...
		LocalPlayer.CharacterAdded:Connect(function() task.wait(2); ClearAuras() end)
		AuraSection:AddDropdown({ Name = "Aura Type", Default = Cfg.AuraType, Multi = false, Flag = "AuraType", Values = {"None", "Gon", "Mob", "Nanami", "Sukuna"}, Callback = ApplyAura })
		AuraSection:AddColorPicker({ Name = "Aura Color", Flag = "AuraColor", Default = Cfg.AuraColor, Callback = function(v) Cfg.AuraColor = v; if not Cfg.RainbowAura then RecolorAuras(v) end end })
		AuraSection:AddToggle({ Name = "Rainbow Aura", Flag = "RainbowAura", Default = Cfg.RainbowAura, Callback = function(v)
			Cfg.RainbowAura = v
			if v then
				if not rainbowAuraConnection or not rainbowAuraConnection.Connected then rainbowAuraConnection = RunService.RenderStepped:Connect(rainbowCycle) end
			else
				if rainbowAuraConnection and rainbowAuraConnection.Connected then rainbowAuraConnection:Disconnect(); rainbowAuraConnection = nil end
				if Cfg.AuraColor then RecolorAuras(Cfg.AuraColor) else ResetAurasToDefault() end
			end
		end})
		AuraSection:AddButton({ Name = "Reset to Default Colors", Flag = "ResetAuraColors", Callback = function() ResetAurasToDefault(); showNotification("Aura System", "Aura colors reset to default!") end })
	end
	do
		local TrailData = { segments = {}, lastPosition = Vector3.new(), movementThreshold = 0.1, connection = nil, rainbowHue = 0, rainbowConnection = nil }
		local function getCurrentColor() return Cfg.RainbowTrail and Color3.fromHSV(TrailData.rainbowHue, 1, 1) or Cfg.TrailColor end
		local function updateRainbow() if Cfg.RainbowTrail then TrailData.rainbowHue = (tick() * 0.1) % 1 end end
		local function createTrailSegment(startPos, endPos)
			local distance = (endPos - startPos).Magnitude
			if distance < TrailData.movementThreshold then return end
			local part = Instance.new("Part", workspace)
			part.Name = "TrailSegment"
			part.Size = Vector3.new(0.2, 0.2, distance)
			part.CFrame = CFrame.lookAt(startPos, endPos) * CFrame.new(0, 0, -distance / 2)
			part.Anchored, part.CanCollide, part.Color, part.Material = true, false, getCurrentColor(), Enum.Material.Neon
		...
		local function stopTrails()
			if TrailData.connection then TrailData.connection:Disconnect(); TrailData.connection = nil end
			for _, segment in ipairs(TrailData.segments) do if segment and segment.Parent then segment:Destroy() end end
			TrailData.segments = {}
		end
		local function startTrails()
			...
			TrailData.connection = RunService.Heartbeat:Connect(function()
				if not Cfg.EnableTrails or not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return stopTrails() end
		...
		TrailsSection:AddToggle({ Name = "Enable Trails", Flag = "EnableTrails", Default = Cfg.EnableTrails, Callback = function(v) Cfg.EnableTrails = v; if v then startTrails() else stopTrails() end end })
		TrailsSection:AddColorPicker({ Name = "Trail Color", Flag = "TrailColor", Default = Cfg.TrailColor, Callback = function(v) Cfg.TrailColor = v end })
		TrailsSection:AddToggle({ Name = "Rainbow Trail", Flag = "RainbowTrail", Default = Cfg.RainbowTrail, Callback = function(v) Cfg.RainbowTrail = v end })
		LocalPlayer.CharacterAdded:Connect(function(char)
			task.wait(1)
			if Cfg.EnableTrails then startTrails() end
		end)
	end
end
do
	local UtilityTab = DualTab5:DrawTab({ Name = "General", Type = "Double", EnableScrolling = true })
	local UtilityLeftSection = UtilityTab:DrawSection({ Name = "General Utils", Position = 'left', EnableScrolling = true })
	local UtilityRightSection = UtilityTab:DrawSection({ Name = "Istant Character Swap/ Server Utilities", Position = 'right', EnableScrolling = true })
	do
		...
		local function teleportToTarget()
			...
			if not currentTarget or not currentTarget.Character or not currentTarget.Character:FindFirstChild("HumanoidRootPart") then return end
			if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
			local targetHRP = currentTarget.Character.HumanoidRootPart
			local targetCF = targetHRP.CFrame
			local targetPos = targetHRP.Position + (targetCF.LookVector * -3)
			local lookAt = CFrame.lookAt(Vector3.new(targetPos.X, targetHRP.Position.Y, targetPos.Z), targetHRP.Position)
		...
		local function startOrbit()
			...
			orbitConnection = RunService.Heartbeat:Connect(function()
				...
				if currentTarget and currentTarget.Character and currentTarget.Character.HumanoidRootPart and LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart then
					...
					require(LocalPlayer.PlayerScripts.Character.FullCustomReplication).Override(LocalPlayer.Character, CFrame.lookAt(newPosition, targetPos))
				end
			end)
		end
			...
			if quickOrbitKeybind and quickOrbitKeybind ~= "NIL" and input.KeyCode.Name == quickOrbitKeybind then
				...
				if isQuickOrbiting then
					...
					if target then
						orbitEnabled = true
						startOrbit()
						showNotification("Utility", "Orbit ON: " .. target.Name)
					else
						isQuickOrbiting = false
						showNotification("Utility", "No valid orbit target")
					end
				else
					orbitEnabled = false; stopOrbit(); showNotification("Utility", "Orbit OFF")
				end
			end
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.KeyCode == Enum.KeyCode.One or input.KeyCode == Enum.KeyCode.Two or input.KeyCode == Enum.KeyCode.Three or input.KeyCode == Enum.KeyCode.Four then
				isTeleporting = true
			end
		end)
		...
		updatePlayerList()
		UtilityLeftSection:AddButton({ Name = "Update Player List", Flag = "UpdatePlayerList", Callback = function()
			local newPlayerNames = updatePlayerList()
			showNotification("Utility", "Player list updated (" .. #newPlayerNames .. " players)")
		end })
		...
		LoopGotoCtrl.Stop = function()
			...
			if LoopGotoCtrl.Conn then LoopGotoCtrl.Conn:Disconnect() LoopGotoCtrl.Conn = nil end
			showNotification("Utility", "Follow stopped")
		end
		LoopGotoCtrl.StartWithTarget = function(target, mode)
			...
			showNotification("Utility", "Following: " .. target.Name)
		end
		UserInputService.InputBegan:Connect(function(input, gp)
			if gp then return end
			if input.KeyCode and input.KeyCode.Name == LoopGotoCtrl.Keybind then
				if not LoopGotoCtrl.IsFollowing then
					...
					if target then
						...
						showNotification("Exploits", "No valid player target")
					end
				else
					LoopGotoCtrl.Stop()
				end
			end
			local quickLoopGotoKeybind = (type(Cfg.QuickLoopGotoKeybind) == "table" and Cfg.QuickLoopGotoKeybind[1]) or Cfg.QuickLoopGotoKeybind
			if input.KeyCode and quickLoopGotoKeybind and quickLoopGotoKeybind ~= "NIL" and input.KeyCode.Name == quickLoopGotoKeybind then
				if not LoopGotoCtrl.IsFollowing or LoopGotoCtrl.Mode ~= "quick" then
					local nearest = findNearestPlayer()
					if nearest then
						LoopGotoCtrl.StartWithTarget(nearest, "quick")
						showNotification("Exploits", "Quick LoopGoto ON: " .. nearest.Name)
					else
						showNotification("Exploits", "No players found!")
					end
				else
					LoopGotoCtrl.Stop()
					showNotification("Exploits", "Quick LoopGoto OFF")
				end
			end
		end)
		...
		UtilityLeftSection:AddDropdown({ Name = "Orbit Target", Flag = "OrbitTarget", Default = Cfg.OrbitTarget, Values = getOrbitPlayerNames(), Multi = false, Callback = function(v) Cfg.OrbitTarget = v; local name = (type(v)=="table" and v[1] or v); showNotification("Utility", "Orbit target: " .. name) end })
		UtilityLeftSection:AddSlider({ Name = "Orbit Speed", Flag = "OrbitSpeed", Default = Cfg.OrbitSpeed, Min = 1, Max = 20, Callback = function(v) Cfg.OrbitSpeed = v end })
		UtilityLeftSection:AddSlider({ Name = "Orbit Radius", Flag = "OrbitRadius", Default = Cfg.OrbitRadius, Min = 5, Max = 50, Callback = function(v) Cfg.OrbitRadius = v end })
		UtilityLeftSection:AddSlider({ Name = "Orbit Height", Flag = "OrbitHeight", Default = Cfg.OrbitHeight, Min = -20, Max = 20, Callback = function(v) Cfg.OrbitHeight = v end })
		local keyOptions = {"NIL", "E", "Q", "R", "T", "Y", "U", "I", "O", "P", "F", "G", "H", "J", "K", "L", "Z", "X", "C", "V", "B", "N", "M"}
		UtilityLeftSection:AddDropdown({ Name = "Orbit Toggle Keybind", Flag = "OrbitKeybind", Default = Cfg.OrbitKeybind, Values = keyOptions, Multi = false, Callback = function(v) Cfg.OrbitKeybind = v end })
	end
	do
		...
		local function changeCharacter(characterName)
			if not Cfg.CharSwapEnabled or charSwapBusy then return end
			charSwapBusy = true
	
			local char = LocalPlayer.Character
			if not char or not char:FindFirstChild("HumanoidRootPart") then charSwapBusy = false; return end
			
			savedSwapPosition = char.HumanoidRootPart.CFrame
			
			if char:GetAttribute("Grabbed") then charSwapBusy = false; return end
			...
			conn = LocalPlayer.CharacterAdded:Connect(function(newChar)
				...
				if savedSwapPosition then
					...
					if rootPart then
						task.wait(0.2)
						require(LocalPlayer.PlayerScripts.Character.FullCustomReplication).Override(newChar, savedSwapPosition)
		...
		local function setupKeybinds()
			for _, c in pairs(charSwapConnections) do c:Disconnect() end
			charSwapConnections = {}
			if not Cfg.CharSwapEnabled then return end
			for charName, key in pairs(keybinds) do
				if key and key ~= "NIL" then
					table.insert(charSwapConnections, UserInputService.InputBegan:Connect(function(input, gp)
						if not gp and input.KeyCode == Enum.KeyCode[key] then changeCharacter(charName) end
					end))
				end
			end
		end
		UtilityRightSection:AddToggle({ Name = "Enable Char Swap", Flag = "CharSwapEnabled", Default = Cfg.CharSwapEnabled, Callback = function(v) Cfg.CharSwapEnabled = v; setupKeybinds(); showNotification("Utility", "Quick Swap: " .. (v and "ON" or "OFF")) end })
		local keyOptions = {"NIL", "E", "Q", "R", "T", "Y", "U", "I", "O", "P", "F", "G", "H", "J", "K", "L", "Z", "X", "C", "V", "B", "N", "M"}
		UtilityRightSection:AddDropdown({ Name = "Gon Key", Flag = "GonKeybind", Default = Cfg.GonKeybind, Multi = false, Values = keyOptions, Callback = function(v) local key = (type(v)=="table" and v[1] or v); Cfg.GonKeybind = v; keybinds.Gon = key; setupKeybinds() end })
		UtilityRightSection:AddDropdown({ Name = "Nanami Key", Flag = "NanamiKeybind", Default = Cfg.NanamiKeybind, Multi = false, Values = keyOptions, Callback = function(v) local key = (type(v)=="table" and v[1] or v); Cfg.NanamiKeybind = v; keybinds.Nanami = key; setupKeybinds() end })
		UtilityRightSection:AddDropdown({ Name = "Mob Key", Flag = "MobKeybind", Default = Cfg.MobKeybind, Multi = false, Values = keyOptions, Callback = function(v) local key = (type(v)=="table" and v[1] or v); Cfg.MobKeybind = v; keybinds.Mob = key; setupKeybinds() end })

		UtilityRightSection:AddButton({ Name = "Rejoin Server", Flag = "RejoinServer", Callback = function() TeleportService:Teleport(game.PlaceId, LocalPlayer) end })
		UtilityRightSection:AddButton({ Name = "Server Hop", Flag = "ServerHop", Callback = function()
			 local servers = game:GetService("HttpService"):JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"))
			 local valid = {}
			 for _, s in pairs(servers.data) do if s.playing < s.maxPlayers and s.id ~= game.JobId then table.insert(valid, s.id) end end
			 if #valid > 0 then TeleportService:TeleportToPlaceInstance(game.PlaceId, valid[math.random(#valid)], LocalPlayer) else showNotification("Utility", "No servers found") end
		end})
	end
end
do
	local ExploitTab = DualTab4:DrawTab({ Name = "Main", Type = "Double", EnableScrolling = true })
	local ExploitSection = ExploitTab:DrawSection({ Name = "Exploits", Position = 'left', EnableScrolling = true })
	
	local LoopGotoSection = ExploitTab:DrawSection({ Name = "Loop Goto", Position = 'right', EnableScrolling = true })
	...
	local _loopPlayerDropdown = LoopGotoSection:AddDropdown({
		...
		Callback = function(v)
			...
			if name then
				showNotification("Exploits", "LoopGoto target: " .. tostring(name))
			end
		end
	})
	...
	LoopGotoSection:AddDropdown({
		...
		Callback = function(v)
			...
			showNotification("Exploits", "LoopGoto toggle key: " .. tostring(key))
		end
	})
	
	LoopGotoSection:AddDropdown({
		Name = "Quick LoopGoto Keybind",
		...
		Callback = function(v)
			...
			showNotification("Exploits", "Quick LoopGoto key: " .. tostring((type(v) == "table" and v[1]) or v))
		end
	})
		...
		Callback = function(v)
			...
			if v then
				...
				if target then
					...
					showNotification("Exploits", "No selected player")
				end
			else
	...
	do
		...
		local function start999Lives()
			...
			healthWatcher = RunService.Heartbeat:Connect(function()
				if not Cfg.InstantRespawn then return end
			...
			respawnHandler = LocalPlayer.CharacterAdded:Connect(function(char)
				if not Cfg.InstantRespawn or not savedPosition then return end
		...
		ExploitSection:AddToggle({ Name = "Scrambled Ping", Flag = "ScrambledPing", Default = Cfg.ScrambledPing, Callback = function(v) 
			Cfg.ScrambledPing = v
			if v then
...
local function autoBlockLoop()
			local remoteEvent = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Combat"):WaitForChild("Block")
		...
		ExploitSection:AddToggle({ Name = "Auto Block", Flag = "AutoBlock", Default = Cfg.AutoBlock, Callback = function(v)
			Cfg.AutoBlock = v
			if v then
            ...
            if self.enabled then
                pcall(function()
                    ...
                    ReplicatedStorage.Remotes.Abilities.AbilityCanceled:FireServer(
                        ReplicatedStorage.Characters[c].Ultimates["4"]
                    )
                end)
            end
        end
...
function AbilitySpam:Stop()
    self.enabled = false
		...
		ExploitSection:AddToggle({ Name = "kill aura v3(gon only and ult)", Flag = "AutoBlock", Default = Cfg.AutoBlock, Callback = function(v)
			Cfg.AutoBlock = v
			if v then
				AbilitySpam:Start()
			else
		...
		local function createPlatform_Invisible()
			...
			if not groundUnion or not playerCharacter then return nil end
		...
		local function enableInvisible()
			...
			require(game.Players.LocalPlayer.PlayerScripts.Character.FullCustomReplication).Override(playerCharacter, targetCFrame)
		...
		local function disableInvisible()
			...
			if playerCharacter then
				...
				workspace.CurrentCamera.CameraSubject = playerCharacter:FindFirstChildOfClass("Humanoid") or playerCharacter:FindFirstChild("HumanoidRootPart")
			end
		...
		ExploitSection:AddToggle({ Name = "Invisible", Flag = "Invisible", Default = Cfg.Invisible, Callback = function(state)
			Cfg.Invisible = state
			if state then
				enableInvisible()
			else
		...
		local function hookCharacter(char, plr)
			...
			hum.AnimationPlayed:Connect(function(track)
				if not Cfg.AntiCounter then return end
		...
		local function filterTable(tbl)
			if not Cfg.AntiCounter or type(tbl) ~= "table" then return tbl end
			for i = #tbl, 1, -1 do
		...
		ExploitSection:AddToggle({ Name = "Anti Counter", Flag = "AntiCounter", Default = Cfg.AntiCounter, Callback = function(v)
			Cfg.AntiCounter = v
			if v then
				initializeAntiCounter()
				showNotification("Exploits", "Anti Counter: ON")
			else
				countering = {}
				showNotification("Exploits", "Anti Counter: OFF")
			end
		end})
		...
		function wallcomboveryez()
			...
			local ultimateBum = npcs:FindFirstChild("Blocking Bum")
		...
		function wallcomboveryez1()
			...
			local remoteArgs = {
				...
				{
					HitboxCFrames = {nil},
					BestHitCharacter = ultimateBum1,
					HitCharacters = {ultimateBum1},
					Ignore = {[actionNumber] = {ultimateBum1}},
					DeathInfo = {},
					Actions = {[actionNumber] = {}},
					HitInfo = {
						Blocked = false,
		...
		function wallcomboveryez2()
			...
			local ultimateBum = npcs:FindFirstChild("Attacking Bum")
			if ultimateBum and ultimateBum:IsA("Model") then
				return ultimateBum
			end
		...
		local function wallcomboveryez3()
			...
			local remoteArgs = {
				...
				{
					HitboxCFrames = {nil},
					BestHitCharacter = ultimateBum3.Character,
					HitCharacters = {ultimateBum3.Character},
					Ignore = {[actionNumber] = {ultimateBum3.Character}},
					DeathInfo = {},
					Actions = {[actionNumber] = {}},
					HitInfo = {
						Blocked = false,
		...
		local function startsGodMode()
			if godModesActive then return end
			godModesActive = true
			...
			showNotification("Exploits", "God Mode: ON")
		end
	
		local function stopsGodMode()
			godModesActive = false
			showNotification("Exploits", "God Mode: OFF")
		end
	
		ExploitSection:AddToggle({ Name = "God Mode1", Flag = "GodMode", Default = Cfg.GodMode or false, Callback = function(v)
			Cfg.GodMode = v
			if v then
				startsGodMode()
			else
...
local function wallcomboveryud()
    ...
    return true
		...
		ExploitSection:AddToggle({ Name = "God Mode2", Flag = "GodMode", Default = Cfg.GodMode or false, Callback = function(v)
			Cfg.GodMode = v
			if v then
				startGodMode()
			else
...
local AbilitySpam5 = {
	...
	function AbilitySpam5:GetCurrentCharacter()
    local ok, res = pcall(function()
        return LocalPlayer.Data.Character.Value
    end)
    if ok and res then return res end
...
function AbilitySpam5:HasAbility4(characterName)
    local ok, res = pcall(function()
        local chars = ReplicatedStorage:WaitForChild("Characters")
        local folder = chars:FindFirstChild(characterName)
        local ab = folder and folder:FindFirstChild("Abilities")
...
UserInputService.InputBegan:Connect(function(input, gp) 
	...
	local function killauraV3click()
		...
		KillAuraSection:AddDropdown({ Name = "Mode", Default =  Cfg.KillAuraV3Mode, Multi = false, Flag = "KillAuraV3Mode", Values = {"Manual", "Spam"}, Callback = function(v) Cfg.KillAuraV3Mode = v; updateKillAuraV3Spam() end })
...
function AbilitySpam3:Stop()
	...
	do
		local ServerLaggerTab = DualTab4:DrawTab({ Name = "Server Lagger", Type = "Single", EnableScrolling = true })
		local ServerLaggerSection = ServerLaggerTab:DrawSection({ Name = "Server Lagger Methods" })
		...
		local function startServerLaggerMethod1()
			if serverLaggerMethod1Active then return end
			...
			showNotification("Server Lagger", "Method 1: ON")
		end
		
		local function stopServerLaggerMethod1()
			if not serverLaggerMethod1Active then return end
			...
			showNotification("Server Lagger", "Method 1: OFF")
		end
		
		ServerLaggerSection:AddToggle({
			Name = "Server Lagger Method 1",
		...
		local function wallcomboMethod2()
			...
			if not ultimateBum then return false end
		...
		local function startServerLaggerMethod2()
			if serverLaggerMethod2Active then return end
			...
			showNotification("Server Lagger", "Method 2: ON (100 Wall Combos/Frame)")
		end
		
		local function stopServerLaggerMethod2()
			if not serverLaggerMethod2Active then return end
			...
			showNotification("Server Lagger", "Method 2: OFF")
		end
		
		ServerLaggerSection:AddToggle({
			Name = "Server Lagger Method 2",
...
function AbilitySpam2:GetCurrentCharacter()
    local ok, res = pcall(function()
        return LocalPlayer.Data.Character.Value
    end)
    if ok and res then return res end
...
function AbilitySpam2:HasAbility4(characterName)
    local ok, res = pcall(function()
        local chars = ReplicatedStorage:WaitForChild("Characters")
        local folder = chars:FindFirstChild(characterName)
        local ab = folder and folder:FindFirstChild("Abilities")
...
function AbilitySpam2:FindNearestPlayer()
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return nil end
...
function AbilitySpam2:GetNearestPlayerCFrame()
    local p = self:FindNearestPlayer()
    return p and p.Character and p.Character.HumanoidRootPart and p.Character.HumanoidRootPart.CFrame or CFrame.new()
end

function AbilitySpam2:UseAbility4()
    local charName = self:GetCurrentCharacter()
    if not self:HasAbility4(charName) then return end
...
function AbilitySpam2:Stop()
    ...
    self.enabled = false
				...
				ServerLaggerSection:AddToggle({
			Name = "Server Lagger Method 3(needs ult and best with gon)",
			Flag = "ServerLaggerMethod3",
			Default = Cfg.ServerLaggerMethod3 or false,
			Callback = function(v)
				Cfg.ServerLaggerMethod3 = v
				if v then
					AbilitySpam2:Start()
				else
					AbilitySpam2:Stop()
...
function AbilitySpam4:Stop()
    ...
    self.enabled = false
				...
				ServerLaggerSection:AddToggle({
			Name = "Server Lagger Method 4(ranked best with mob)",
			Flag = "ServerLaggerMethod4",
			Default = Cfg.ServerLaggerMethod4 or false,
			Callback = function(v)
...
-- Anti Server Lagger
		...
		local function startAnti4ServerLagger()
			if antiSLEnabled then return end
			antiSLEnabled = true
		...
		local function stopAnti4ServerLagger()
			if not antiSLEnabled then return end
			antiSLEnabled = false
		...
		ServerLaggerSection:AddToggle({
			Name = "Anti Server Lagger(old)",
			Flag = "Anti4ServerLagger",
			Default = Cfg.Anti4ServerLagger or false,
			Callback = function(v)
				Cfg.Anti4ServerLagger = v
				if v then
					startAnti4ServerLagger()
				else
					stopAnti4ServerLagger()
				end
			end
		})
...
--game:GetService("ReplicatedStorage").Characters.Gon.WallCombo:FindFirstChild("GonWallCombo")
    --game:GetService("SoundService").All.Sounds:FindFirstChild("GonWallCombo")
local function startGonAntiServerLagger()
	removeInstances(
		game:GetService("ReplicatedStorage").Characters.Mob.Abilities["4"].MobCounter,
		game:GetService("ReplicatedStorage").Characters.Mob.Abilities["4"].MobCounterAttack,
		game:GetService("ReplicatedStorage").Characters.Mob.Abilities["4"].MobCounterBubble,
		...
		game:GetService("ReplicatedStorage").Characters.Stark.Abilities["4"].StarkBrilliant,
		game:GetService("ReplicatedStorage").Characters.Stark.WallCombo.StarkWallCombo,
		game:GetService("ReplicatedStorage").Characters.Sukuna.WallCombo.SukunaTransformWallCombo,
		game:GetService("ReplicatedStorage").Characters.Sukuna.WallCombo.SukunaWallCombo,
		game:GetService("ReplicatedStorage").Characters.Sukuna.Ultimates["4"].MalevolentShrine,
		game:GetService("ReplicatedStorage").Characters.Sukuna.Abilities["4"].SukunaSpiderweb,
		game:GetService("ReplicatedStorage").Characters.Mob.WallCombo:FindFirstChild("MobWallCombo"))

    
    showNotification("Server Lagger", "Anti Server Lagger: ON")
end

local function stopGonAntiServerLagger()
	restoreInstances()
    showNotification("Server Lagger", "Anti Server Lagger: OFF")
end
		
		ServerLaggerSection:AddToggle({
			Name = "antiserverlagger for gon (breaks wallcombo and ability4 and ult4 for everyone but gon)",
			Flag = "AntiServerLaggerGon",
			Default = Cfg.AntiServerLaggerGon or false,
			Callback = function(v)
				Cfg.AntiServerLaggerGon = v
				if v then
					startGonAntiServerLagger()
				else
					stopGonAntiServerLagger()
				end
			end
		})
		...
		ServerLaggerSection:AddToggle({
			Name = "Anti Server Lagger mob counter(breaks server lagger 4)",
			Flag = "Anti2ServerLagger",
			Default = Cfg.Anti2ServerLagger or false,
			Callback = function(v)
				Cfg.Anti2ServerLagger = v
				if v then
					startAnti2ServerLagger()
				else
					stopAnti2ServerLagger()
				end
			end
		})
		...
		ServerLaggerSection:AddToggle({
			Name = "Anti Server Lagger wallcombo2(breaks nanami wallcombo)",
			Flag = "Anti3ServerLagger",
			Default = Cfg.Anti3ServerLagger or false,
			Callback = function(v)
				Cfg.Anti3ServerLagger = v
				if v then
					startAnti3ServerLagger()
				else
...
do
	...
	local Section2 = CombatTab1:DrawSection({ Name = "Instant Kill", Position = 'right', EnableScrolling = true })
	local CombatTab2 = DualTab1:DrawTab({ Name = "tab 2", Type = "Double", EnableScrolling = true })
	local SpecialSection1 = CombatTab2:DrawSection({ Name = "Wall Combo", Position = 'left', EnableScrolling = true })
	local SpecialSection2 = CombatTab2:DrawSection({ Name = "Kill Emote", Position = 'right', EnableScrolling = true })
	...
	pcall(function() core = require(ReplicatedStorage:WaitForChild("Core", 9e9)) end)
	local function initializeHitbox()
		if core and not originalHitBox then
			local combat = core.Get("Combat", "Hit")
			if combat and combat.Box then originalHitBox = combat.Box end
		end
	end
	local function toggleHitbox(enabled)
		...
		if enabled then
			if not originalHitBox then initializeHitbox() end
			combat.Box = function(...)
				local args = {...}
				...
				if getgenv().HitboxLegitify then
					...
					if getgenv().isExcludedCharacter and Cfg.HitboxIgnoreFriends then
	...
	Section1:AddToggle({ Name = "Enable", Flag = "HitboxToggle", Default = Cfg.HitboxToggle, Callback = function(v) Cfg.HitboxToggle = v; toggleHitbox(v) end })
	local gurt = Section1:AddToggle({ Name = "Legitify", Flag = "HitboxLegitify", Default = Cfg.HitboxLegitify, Callback = function(v) Cfg.HitboxLegitify = v; getgenv().HitboxLegitify = v; if getgenv().HitboxEnabled then toggleHitbox(false); toggleHitbox(true) end end })
	local wownd = Section1:AddDropdown({ Name = "Method", Flag = "HitboxMethod", Default = Cfg.HitboxMethod, Multi = false, Values = {"Override", "Add"}, Callback = function(v) Cfg.HitboxMethod = v; getgenv().HitboxMethod = (type(v)=="table" and v[1] or v); if getgenv().HitboxEnabled then toggleHitbox(false); toggleHitbox(true) end end })
	gurt.Link:AddHelper({
	...
	do
		...
		local function enableEnhancer()
			...
			local ok_hit, hit = pcall(function() return require(PlayersService.LocalPlayer.PlayerScripts.Combat.Hit) end)
		...
		local function disableEnhancer()
			if originalHitProcess then
				local ok, hit = pcall(function() return require(game:GetService("Players").LocalPlayer.PlayerScripts.Combat.Hit) end)
				if ok then
		...
		enhancerToggle.Link:AddHelper({
			Text = "this will make the hitbox expander 10 times better, buff damage, and remove the first hit requirement.\nset the enchancer value to 0 in order to remove the damage buff.\neach value will buff you 2% of damage by the way"
		})
		
		Section1:AddSlider({
			Name = "Enhancer Multiplier",
	...
	local function addSizeSlider(name, flag, sizeVar)
		Section1:AddSlider({ Name = name, Min = 1, Max = 250, Default = Cfg[flag], Round = 0, Flag = flag, Callback = function(v) 
			Cfg[flag] = v; 
			getgenv()[sizeVar] = v;
			if getgenv().HitboxEnabled then toggleHitbox(false); toggleHitbox(true) end 
		end })
	end	
	...
	Section1:AddToggle({ Name = "Hitbox Visualizer", Flag = "HitboxVisualizer", Default = Cfg.HitboxVisualizer, Callback = function(v) Cfg.HitboxVisualizer = v; getgenv().HitboxVisualizer = v end })
	Section1:AddToggle({ Name = "Ignore Friends", Flag = "HitboxIgnoreFriends", Default = Cfg.HitboxIgnoreFriends, Callback = function(v) Cfg.HitboxIgnoreFriends = v end})
	...
	do
		...
		local function createRangeCircle()
			...
			for i = 1, segments do
				...
				local pos1_offset = Vector3.new(math.cos(angle1) * Cfg.RangeRadius, 0.1, math.sin(angle1) * Cfg.RangeRadius)
				local pos2_offset = Vector3.new(math.cos(angle2) * Cfg.RangeRadius, 0.1, math.sin(angle2) * Cfg.RangeRadius)
		...
		local function updateRangeCircle()
			if not Cfg.ShowRangeCircle or not rangeCircle then return end
			local hrp = getHRP()
			if not hrp then
		...
		local function getTargets(selectionMode, ignoreFriends, range)
			...
			for _, v in ipairs(game.Players:GetPlayers()) do
				...
				if character and v ~= LocalPlayer and v:FindFirstChild(' ') == nil then
					if ignoreFriends and isFriend(v) then continue end
					...
					if humanoid and humanoidRootPart then
						...
						if distance < range then
							local function validateTarget(character)
								...
								if not localPlayerChar then return false end
								local localPlayerName = localPlayerChar.Name
								
								local cutscene = character:GetAttribute("Cutscene")
								local cutsceneValid = not cutscene or cutscene == localPlayerChar:GetAttribute("Cutscene")
								
								local isValid = humanoidRootPart and 
									(not character:GetAttribute("Ignore") and 
									cutsceneValid and 
									((not character:GetAttribute("Grabbed") or character:GetAttribute("Grabbed") == localPlayerName) and 
									(not character:GetAttribute("Victim") or character:GetAttribute("Victim") == localPlayerName)) and 
									not character:GetAttribute("Invincible") and 
		...
		local function KillAura(n)
			...
			local targetSelection = (type(Cfg.InstantKillTargetSelection) == "table" and Cfg.InstantKillTargetSelection[1]) or Cfg.InstantKillTargetSelection or "Closest"
			...
			if index>1 then
				...
				game.ReplicatedStorage.Remotes.Combat.Action:FireServer(CurrentWallCombo,"",4,69,{BestHitCharacter=nil,HitCharacters=PlayersList,Ignore={},Actions={}})
		...
		Section2:AddToggle({ Name = "Instant Kill", Flag = "InstantKill", Default = Cfg.InstantKill, Callback = function(enabled)
		...
		Section2:AddToggle({ Name = "Show Range Circle", Flag = "ShowRangeCircle", Default = Cfg.ShowRangeCircle, Callback = toggleRangeVisualizer })
		Section2:AddColorPicker({ Name = "Range Circle Color", Flag = "RangeCircleColor", Default = Cfg.RangeCircleColor, Callback = function(color) Cfg.RangeCircleColor = color end })
		Section2:AddSlider({
			Name = "Range Radius",
			Flag = "RangeRadius",
			Min = 1,
			...
			Callback = function(value)
				local clamped = math.clamp(value, 1, 500)
		...
		local function startKillFarming()
			if Cfg.KillFarming then return end
		...
		local function stopKillFarming()
			if not Cfg.KillFarming then return end
		...
		Section2:AddToggle({ Name = "Kill Farming", Flag = "KillFarming", Default = Cfg.KillFarming, Callback = function(v)
			if v then startKillFarming() else stopKillFarming() end
		end })
	end
	...
	do
		...
		local function run_as_identity(level, fn)
			...
			if set_id then pcall(set_id, level) end
		...
		local function wallcomboveryud()
			...
			local hitRes = run_as_identity(2, function()
				return core.Get("Combat","Hit").Box(nil, playerChar, {Size = Vector3.new(wallComboSize, wallComboSize, wallComboSize)})
			end)
			...
			if hitRes then
				run_as_identity(2, function()
					pcall(core.Get("Combat","Ability").Activate, charsFolder[charValue].WallCombo, hitRes, head.Position + Vector3.new(0,0,2.5))
				end)
			end
			return true
		end
		...
		local function updateWallComboSpam()
			if wallComboConnection then wallComboConnection:Disconnect(); wallComboConnection = nil end
		...
		UserInputService.InputBegan:Connect(function(input, gp) 
			local wallComboKeybind = (type(Cfg.WallComboKeybind) == "table" and Cfg.WallComboKeybind[1]) or Cfg.WallComboKeybind
			local wallComboMode = (type(Cfg.WallComboMode) == "table" and Cfg.WallComboMode[1]) or Cfg.WallComboMode
			if not gp and wallComboKeybind and wallComboKeybind ~= "NIL" and input.KeyCode.Name == wallComboKeybind and Cfg.WallCombo then
	...
	do
		...
		local function useEmote(emoteName)
			if not Cfg.KillEmoteEnabled or not emoteName or tick() - lastUse < emoteCooldown then return end
			lastUse = tick()
			...
			for _, player in pairs(Players:GetPlayers()) do
				if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
					...
					if Cfg.KillEmoteIgnoreFriends and isFriend(player) then continue end
					
					local distance = (HRP.Position - player.Character.HumanoidRootPart.Position).Magnitude
					local targetSelection = (type(Cfg.KillEmoteTargetSelection) == "table" and Cfg.KillEmoteTargetSelection[1]) or Cfg.KillEmoteTargetSelection or "Closest"
		...
		UserInputService.InputBegan:Connect(function(input, isProcessed)
			...
			local selectedKeybind = (selectedKeybindName and selectedKeybindName ~= "NIL") and Enum.KeyCode[selectedKeybindName] or nil
			...
			local selectedEmote = (type(Cfg.KillEmoteSelection) == "table" and Cfg.KillEmoteSelection[1]) or Cfg.KillEmoteSelection
		...
		local function startKillEmoteLoop()
			if killEmoteHeartbeatConn and killEmoteHeartbeatConn.Connected then return end
			killEmoteHeartbeatConn = RunService.Heartbeat:Connect(function()
				local now = tick()
				if not Cfg.KillEmoteEnabled then return end
				...
				if isSpammingSelectedEmote and not isAuraMode and selectedEmote and selectedEmote ~= "" and now - lastSpam >= Cfg.KillEmoteDelay then
					useEmote(selectedEmote)
					lastSpam = now
				end
			end)
		...
		SpecialSection2:AddToggle({ Name = "Enable Kill Emote", Flag = "KillEmoteEnabled", Default = Cfg.KillEmoteEnabled, Callback = function(v)
		...
		SpecialSection2:AddDropdown({ Name = "Kill Emote", Default = Cfg.KillEmoteSelection, Multi = false, Flag = "KillEmoteSelection", Values = killEmotes, Callback = function(v) Cfg.KillEmoteSelection = v end })
	
		SpecialSection2:AddDropdown({ Name = "Mode", Default = Cfg.KillEmoteMode, Multi = false, Flag = "KillEmoteMode", Values = {"Selected", "Randomized"}, Callback = function(v) 
			Cfg.KillEmoteMode = v
			isAuraMode, isSpammingSelectedEmote = false, false
			if Cfg.KillEmoteEnabled then startKillEmoteLoop() end
		end })
	
		SpecialSection2:AddDropdown({ Name = "Target Selection", Default = Cfg.KillEmoteTargetSelection, Multi = false, Flag = "KillEmoteTargetSelection", Values = {"Closest", "Lowest"}, Callback = function(v) Cfg.KillEmoteTargetSelection = v end })
		SpecialSection2:AddToggle({ Name = "Ignore Friends", Flag = "KillEmoteIgnoreFriends", Default = Cfg.KillEmoteIgnoreFriends, Callback = function(v) Cfg.KillEmoteIgnoreFriends = v end })
		
		SpecialSection2:AddDropdown({ Name = "Type", Default = Cfg.KillEmoteType, Multi = false, Flag = "KillEmoteType", Values = {"Manual", "Spam"}, Callback = function(v) 
			Cfg.KillEmoteType = v
			isAuraMode, isSpammingSelectedEmote = false, false
		...
		SpecialSection2:AddSlider({ Name = "Spam Delay", Min = 0.01, Max = 5.0, Default = Cfg.KillEmoteDelay, Round = 2, Flag = "KillEmoteDelay", Callback = function(v) Cfg.KillEmoteDelay = v end })
		SpecialSection2:AddDropdown({ Name = "Manual Keybind", Default = Cfg.EmoteKeybind, Multi = false, Flag = "EmoteKeybind", Values = {"NIL","E","Q","R","T","Y","G","F","C","V","B"}, Callback = function(v) Cfg.EmoteKeybind = v end})
	end
						...
						local v182 = {
							...
							Replace = function(p168)
								...
								if isRagdolled then
									if DEBUG then
										pcall(function()
											null(string.format("[Replace] RAGDOLL -> forcing {2,6} | elapsed=%.4f thr=%.4f hasKB=%s isRagdolled=%s", v_u_115, v169, tostring(hasKB), tostring(isRagdolled)))
										end)
									end
								...
								if v_u_115 < v169 then
									...
									if DEBUG then
										pcall(function()
											null(string.format("[Replace] EARLY non-ragdoll -> forcing {1,6} (no KB) | elapsed=%.4f thr=%.4f hasKB=%s", v_u_115, v169, tostring(hasKB)))
										end)
									end
								...
								if DEBUG then
									pcall(function()
										null(string.format("[Replace] FALLBACK -> pick=%d | elapsed=%.4f thr=%.4f hasKB=%s isRagdolled=%s", pick, v_u_115, v169, tostring(hasKB), tostring(isRagdolled)))
									end)
								end
	...
	local function enableLegitKombat()
		if runAttackHooked then return end
	...
	local function disableLegitKombat()
		if not runAttackHooked or not originalRunAttack or not targetRunAttackFunc then return end
...
do
	local UISettingsTab = Window:DrawTab({ Name = "UI Settings", Icon = "settings-3", Type = "Single", EnableScrolling = true })
	local UISettingsSection = UISettingsTab:DrawSection({ Name = "UI Settings" })
	UISettingsSection:AddToggle({ Name = "Always Show Frame", Flag = "AlwaysShowFrame", Default = false, Callback = function(v) Window.AlwayShowTab = v end })
	UISettingsSection:AddColorPicker({ Name = "Highlight Color", Flag = "HighlightColor", Default = Compkiller.Colors.Highlight, Callback = function(v) Compkiller:ChangeHighlightColor(v); Compkiller:RefreshCurrentColor() end })
	UISettingsSection:AddColorPicker({ Name = "Toggle Color", Flag = "ToggleColor", Default = Compkiller.Colors.Toggle, Callback = function(v) Compkiller.Colors.Toggle = v; Compkiller:RefreshCurrentColor() end })
	UISettingsSection:AddColorPicker({ Name = 
"Drop Color", Flag = "DropColor", Default = Compkiller.Colors.DropColor, Callback = function(v) Compkiller.Colors.DropColor = v;
Compkiller:RefreshCurrentColor() end })
	UISettingsSection:AddColorPicker({ Name = "Background Color", Flag = "BackgroundColor", Default = Compkiller.Colors.BGDBColor, Callback = function(v) Compkiller.Colors.BGDBColor = v; Compkiller:RefreshCurrentColor() end })
	UISettingsSection:AddColorPicker({ Name = "Stroke Color", Flag = "StrokeColor", Default = Compkiller.Colors.StrokeColor, Callback = function(v) Compkiller.Colors.StrokeColor = v; Compkiller:RefreshCurrentColor() end })
	UISettingsSection:AddDropdown({ Name = "Select Theme", Flag = "SelectTheme", Default = "Default", Values = {"Default", "Dark Green", "Dark Blue", "Purple Rose", "Skeet"}, Callback = function(v) Compkiller:SetTheme(v) end })
	init7 = true
end


repeat task.wait(2) until init7
